...
class MetarInfoScreen(ModalScreen):
    ...
    def __init__(self, initial_icao: str | None = None):
        ...
        self.current_icao: str | None = (
            None  # Track current airport for VFR alternatives (used by global Ctrl+A)
        )
    ...
    def compose(self) -> ComposeResult:
        with Container(id="metar-container"):
            ...
            with Container(id="metar-input-container"):
                yield Input(
                    placeholder="Enter airport ICAO code (e.g., KSFO)", id="metar-input"
                )
    ...
    def _parse_taf_time(
        ...
        try:
            ...
            return datetime(
                current_year, current_month, day, hour, minute, tzinfo=timezone.utc
            )
    ...
    def _highlight_flight_category_components(self, metar: str) -> str:
        ...
        if vis_str:
            highlighted = highlighted.replace(
                vis_str,
                f"[bold underline {color}]{vis_str}[/bold underline {color}]",
                1,
            )
        ...
        if ceiling_str:
            highlighted = highlighted.replace(
                ceiling_str,
                f"[bold underline {color}]{ceiling_str}[/bold underline {color}]",
                1,
            )
    ...
    def _colorize_taf(self, taf: str) -> str:
        ...
        for line in lines:
            ...
            if header_match:
                ...
                valid_from = self._parse_taf_time(
                    valid_from_str, current_month, current_year
                )
                valid_to = self._parse_taf_time(
                    valid_to_str, current_month, current_year
                )
                ...
                if valid_from and valid_to and valid_from <= current_time <= valid_to:
                    ...
                    colorized_lines.append(
                        f"{line[:period_end]}{relative_from}[bold yellow]{conditions}[/bold yellow]"
                    )
                else:
            ...
            if fm_match:
                ...
                fm_end = fm_match.end()
                annotated_line = (
                    line[: fm_end - 1] + relative_time + " " + line[fm_end:]
                )
                ...
                if fm_time:
                    ...
                    if 0 <= time_diff <= 86400:  # Within 24 hours from FM time
                        ...
                        colorized_lines.append(
                            f"{line[: fm_end - 1]}{relative_time} [bold yellow]{conditions}[/bold yellow]"
                        )
                    else:
                        colorized_lines.append(annotated_line)
                else:
            ...
            if tempo_becmg_match:
                ...
                if valid_from and valid_to and valid_from <= current_time <= valid_to:
                    ...
                    colorized_lines.append(
                        f"{line[:group_end]}{relative_from}[bold yellow]{conditions}[/bold yellow]"
                    )
                else:
                    colorized_lines.append(annotated_line)
                continue
    ...
    async def _fetch_metar_async(self, icao: str, pretty_name: str) -> None:
        ...
        metar, taf, vatsim_data = await asyncio.gather(
        ...
        if metar:
            ...
            result_lines.append(
                f"{pretty_name} ({icao}) // [{color} bold]{category}[/{color} bold]"
            )
            result_lines.append("")
        else:
        ...
        if metar:
            ...
            if is_speci_metar(metar):
                result_lines.append(
                    "[bold #ff9900]⚠ SPECI[/bold #ff9900] [dim](significant weather change)[/dim]"
                )
    ...
    def _update_hint(self, category: str | None) -> None:
        ...
        if category in ("IFR", "LIFR"):
            hint_widget.update(
                "Press Enter to fetch, [bold]Ctrl+A[/bold] for VFR alternatives, Escape to close"
            )
        else:
