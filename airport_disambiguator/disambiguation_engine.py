...
class DisambiguationEngine:
    ...
    def disambiguate_single_airport(
        ...
        entity = self.entity_extractor.extract_distinguishing_entity(
        ...
        non_high_priority_words = self.name_processor.get_non_high_priority_prefix(
    ...
    def disambiguate_multiple_airports(
        ...
        airport_names = {
            icao: self.name_processor.shorten_name(
                airports_data[icao].get("name", icao)
            )
            for icao in icaos
        }
    ...
    def _disambiguate_non_location_start(
        ...
        if not self.name_processor.name_contains_location(full_name, city, state):
            ...
            non_high_priority = self.name_processor.get_non_high_priority_prefix(
                full_name, location
            )
        ...
        distinguishing_parts = self.name_processor.extract_distinguishing_words(
            shortened_name, location
        )
        high_priority_word = self.name_processor.find_first_high_priority_word(
    ...
    def _disambiguate_location_starts(
        ...
        for icao in icaos:
            ...
            for word in proper_names:
                ...
                if self._is_unique_in_group_optimized(
                    candidate, icao, icaos, location_words, all_distinguishing_sets
                ):
    ...
    def _try_combinations_with_priority(
        ...
        for num_words in range(2, len(distinguishing_parts) + 1):
            for start_idx in range(len(distinguishing_parts) - num_words + 1):
                candidate_words = distinguishing_parts[
                    start_idx : start_idx + num_words
                ]
                ...
                if has_high_priority:
                    ...
                    if self._is_unique_in_group(
                        candidate, icao, all_icaos, location, airport_names
                    ):
                        resolved_names[icao] = candidate
                        return True
    ...
    def _is_unique_in_group_optimized(
        self,
        candidate_name: str,
        current_icao: str,
        all_icaos: List[str],
    ...
    def _try_sequential_combinations_optimized(
        ...
        for i in range(1, len(distinguishing_parts) + 1):
            candidate = f"{location} {' '.join(distinguishing_parts[:i])}".strip()
            if self._is_unique_in_group_optimized(
                candidate, icao, all_icaos, location_words, all_distinguishing_sets
            ):
                resolved_names[icao] = candidate
                return True
