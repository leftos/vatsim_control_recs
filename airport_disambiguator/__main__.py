...
def verbose_trace_disambiguation(icao: str, unified_data: dict, disambiguator: AirportDisambiguator) -> str:
    ...
    if dm_airport:
        ...
        print(f"         NOT FOUND in data_manager")
    ...
    if dm_airport:
        ...
        if city:
            ...
            if not found_match:
                print(f"         NO MATCH found between name_words and city_words")

    # Step 6: Check what's in the caches
    print(f"\n[STEP 6] Disambiguation caches (before get_pretty_name):")
    print(f"         icao_to_pretty_name has {icao}: {icao in disambiguator.icao_to_pretty_name}")
    print(f"         icao_to_full_name has {icao}:   {icao in disambiguator.icao_to_full_name}")
    if icao in disambiguator.icao_to_pretty_name:
        print(f"         cached pretty_name: {disambiguator.icao_to_pretty_name[icao]!r}")
    ...
    print(f"         icao_to_pretty_name[{icao}]: {disambiguator.icao_to_pretty_name.get(icao)!r}")
    print(f"         icao_to_full_name[{icao}]:   {disambiguator.icao_to_full_name.get(icao)!r}")
...
Examples:
  ...
  python -m airport_disambiguator --apt-base data/APT_BASE.csv --airports data/airports.json --iata-icao data/iata-icao.csv KSFO KLAX
    ...
    try:
        ...
        if args.verbose:
            print(f"Disambiguator data_manager has {len(disambiguator.data_manager.airports_data)} airports")
            print(f"Disambiguator has {len(disambiguator.data_manager.location_to_airports)} unique locations")
        ...
        for icao in args.icao_codes:
            if args.verbose:
                pretty_name = verbose_trace_disambiguation(icao, unified_data, disambiguator)
            else:
                pretty_name = disambiguator.get_pretty_name(icao)
