...
class NameProcessor:
    ...
    def extract_distinguishing_words(
        ...
        for word in name.split():
            ...
            if "/" in cleaned_word or "-" in cleaned_word:
                ...
                non_location_parts = [
                    p for p in parts if p.lower() not in location_words
                ]
    ...
    def get_military_name(self, airport_name: str, location: str) -> Optional[str]:
        ...
        if not military_phrase:
            for word in name.split():
                if (
                    word in self.config.ALWAYS_INCLUDE_WORDS
                    or word.lower().capitalize() in self.config.ALWAYS_INCLUDE_WORDS
                ):
                    military_word = word
                    break
        ...
        for word in words:
            ...
            if any(
                part.lower() in self.config.GENERIC_WORDS_FOR_MILITARY
                for part in self._split_compound_word(word)
            ):
                continue
