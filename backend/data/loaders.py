            ...
            for code, details in airports_json_data.items():
                ...
                if icao in airports:
                    airports[icao].update(
                        {
                            "name": safe_strip(
                                details.get("name"), airports[icao]["name"]
                            ),
                            "city": safe_strip(
                                details.get("city"), airports[icao]["city"]
                            ),
                            "state": safe_strip(
                                details.get("state"), airports[icao]["state"]
                            ),
                            "country": safe_strip(
                                details.get("country"), airports[icao]["country"]
                            ),
                            "latitude": details.get("lat", airports[icao]["latitude"]),
                            "longitude": details.get(
                                "lon", airports[icao]["longitude"]
                            ),
                            "elevation": details.get(
                                "elevation", airports[icao]["elevation"]
                            ),
    ...
    try:
        with open(apt_base_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                try:
                    lat_decimal = (
                        float(row.get("LAT_DECIMAL", ""))
                        if row.get("LAT_DECIMAL")
                        else None
                    )
                    long_decimal = (
                        float(row.get("LONG_DECIMAL", ""))
                        if row.get("LONG_DECIMAL")
                        else None
                    )
                ...
                if primary_code in airports:
                    ...
                    if (
                        country in ("US", "USA") and existing_country in ("US", "USA")
                    ) or (
                        country not in ("US", "USA")
                        and existing_country not in ("US", "USA")
                    ):
                        # Both same country preference - update with APT_BASE data but keep existing info
                        pass
                    elif country in ("US", "USA") and existing_country not in (
                        "US",
                        "USA",
                    ):
                        # New one is US, existing is not - replace with US airport
                        pass
                    elif existing_country in ("US", "USA") and country not in (
                        "US",
                        "USA",
                    ):
                ...
                airport_data.update(
                    {
                        ...
                        "faa": arpt_id,
                        "name": row.get(
                            "ARPT_NAME", airport_data.get("name", "")
                        ).strip(),
                        "city": row.get("CITY", airport_data.get("city", "")).strip(),
                        "state": row.get(
                            "STATE_NAME", airport_data.get("state", "")
                        ).strip(),
                        "country": country or airport_data.get("country", ""),
                        "latitude": lat_decimal
                        if lat_decimal is not None
                        else airport_data.get("latitude"),
                        "longitude": long_decimal
                        if long_decimal is not None
                        else airport_data.get("longitude"),
                        "elevation": elevation
                        if elevation is not None
                        else airport_data.get("elevation"),
